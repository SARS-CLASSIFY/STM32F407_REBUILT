...
void OLED1_ColorTurn(u8 i)
{
	if(i==0)
		{
			OLED1_WR_Byte(0xA6,OLED1_CMD);//正常显示
		}
	if(i==1)
		{
			OLED1_WR_Byte(0xA7,OLED1_CMD);//反色显示
		}
}
...
void OLED1_DisplayTurn(u8 i)
{
	if(i==0)
		{
			OLED1_WR_Byte(0xC8,OLED1_CMD);//正常显示
			OLED1_WR_Byte(0xA1,OLED1_CMD);
		}
	if(i==1)
		{
			OLED1_WR_Byte(0xC0,OLED1_CMD);//反转显示
...
//等待信号响应
void I2C1_WaitAck(void) //测数据信号的电平
{
	OLED1_SDA_Set();
	IIC1_delay();
	OLED1_SCL_Set();
...
{
	OLED1_WR_Byte(0x8D,OLED1_CMD);//电荷泵使能
	OLED1_WR_Byte(0x14,OLED1_CMD);//开启电荷泵
	OLED1_WR_Byte(0xAF,OLED1_CMD);//点亮屏幕
}
...
{
	OLED1_WR_Byte(0x8D,OLED1_CMD);//电荷泵使能
	OLED1_WR_Byte(0x10,OLED1_CMD);//关闭电荷泵
	OLED1_WR_Byte(0xAE,OLED1_CMD);//关闭屏幕
}
...
void OLED1_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		OLED1_WR_Byte(0xb0+i,OLED1_CMD); //设置行起始地址
		OLED1_WR_Byte(0x00,OLED1_CMD);   //设置低列起始地址
		OLED1_WR_Byte(0x10,OLED1_CMD);   //设置高列起始地址
		I2C1_Start();
		Send1_Byte(0x78);
		I2C1_WaitAck();
		Send1_Byte(0x40);
...
void OLED1_DrawLine(u8 x1,u8 y1,u8 x2,u8 y2,u8 mode)
{
	...
	{
		OLED1_DrawPoint(uRow,uCol,mode);//画点
		xerr+=delta_x;
		yerr+=delta_y;
		if(xerr>distance)
		{
...
void OLED1_ShowChar(u8 x,u8 y,u8 chr,u8 size1,u8 mode)
{
	...
	{
		if(size1==8)
			  {temp=asc2_08061[chr1][i];} //调用0806字体
		else if(size1==12)
        {temp=asc2_12061[chr1][i];} //调用1206字体
		else if(size1==16)
        {temp=asc2_16081[chr1][i];} //调用1608字体
		else if(size1==24)
        {temp=asc2_24121[chr1][i];} //调用2412字体
		else return;
		for(m=0;m<8;m++)
		{
...
void OLED1_ShowChinese(u8 x,u8 y,u8 num,u8 size1,u8 mode)
{
	...
	for(i=0;i<size3;i++)
	{
		if(size1==16)
				...
				{temp=Hzk21[num][i];}//调用24*24字体
		else if(size1==32)       
				{temp=Hzk31[num][i];}//调用32*32字体
		else if(size1==64)
				{temp=Hzk41[num][i];}//调用64*64字体
		else return;
		for(m=0;m<8;m++)
		{
			if(temp&0x01)OLED1_DrawPoint(x,y,mode);
...
void OLED1_ScrollDisplay(u8 num,u8 space,u8 mode)
{
	...
	while(1)
	{
		if(m==0)
		{
	    OLED1_ShowChinese(128,24,t,16,mode); //写入一个汉字保存在OLED1_GRAM[][]数组中
			t++;
		}
		if(t==num)
			{
...
//	GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);//初始化
	...
	OLED1_WR_Byte(0xA1,OLED1_CMD);//--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
	OLED1_WR_Byte(0xC8,OLED1_CMD);//Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
...
{	
	OLED1_Clear_Sp();
//	OLED1_Init();				//初始化OLED
	OLED1_ShowChinese(0,0,0,16,1);//频
	OLED1_ShowChinese(16,0,1,16,1);//率
	OLED1_ShowChar(32,0,':',16,1);
	
	OLED1_ShowChinese(0,18,2,16,1);//占
	OLED1_ShowChinese(16,18,3,16,1);//空
	OLED1_ShowChinese(32,18,4,16,1);//比

...
{		 					 
	...
	//开启多个引脚的复用功能
	GPIO_PinAFConfig(GPIOA,GPIO_PinSource7,GPIO_AF_TIM3); //GPIOA6复用为定时器3				***>___<***
...
/**************失能PWM生成***********************************************
** TIM_TypeDef* TIMx :		定时器X
**brief :使用配置CCR寄存器来实现PWM输出结束时一定处于低电平状态,若直接失能
		定时器会很不稳定
************************************************************************/
void close_pwm(TIM_TypeDef* TIMx,u8 channel)
{
	if(channel&0x08){
		TIM_SetCompare1(TIMx,0xffff);
	}
	if(channel&0x04){
		TIM_SetCompare2(TIMx,0xffff);
	}
	if(channel&0x02){
		TIM_SetCompare3(TIMx,0xffff);
	}
	if(channel&0x01){
		TIM_SetCompare4(TIMx,0xffff);
	}
}
